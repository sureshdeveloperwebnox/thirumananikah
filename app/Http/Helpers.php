...
// get member additional info
if (!function_exists('additional_member_info')) {
    function additional_member_info($member_id, $attribute_id)
    {
        $additional_member_info = AdditionalMemberInfo::where('user_id', $member_id)->where('additional_attribute_id', $attribute_id)->first();
        return $additional_member_info != null ? $additional_member_info->value : null;
    }
}
...
if (!function_exists('static_avatar')) {
    function static_avatar($user)
    {
        if ($user) {
            if ($user instanceof \App\Models\User) {
                if ($user->member && $user->member->gender == 2) {
                    return static_asset('assets/img/female-avatar-place.png');
                }
