<div class="rounded border position-relative overflow-hidden">
	...
	>
		...
		@endphp
		<img
				@if($profile_picture_show && $member->photo != null)
					src="{{ uploaded_asset($member->photo) }}"
				@else
					src="{{ static_avatar($member) }}"
				@endif
				onerror="this.onerror=null;this.src='{{ static_avatar($member) }}';"
				class="img-fit mw-100 h-350px"
		>
		@if(!$profile_picture_show)
			<div class="absolute-full d-flex justify-content-center align-items-center bg-soft-dark1 text-white"><i class="las la-lock la-3x"></i></div>
			<style>
			    .bg-soft-dark1{
			        background:#0000006b;
			    }
