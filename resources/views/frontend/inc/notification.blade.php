                ...
                <li class="list-group-item d-flex justify-content-between align-items-start hov-bg-soft-primary">
                    <a href="{{ route('notification_view', $notification->id) }}" class="media text-inherit">
                        <span class="avatar avatar-sm mr-3">
                            ...
                            @endphp
                            <img @if ($profile_picture_show && $user_data->photo != null) src="{{ uploaded_asset($user_data->photo) }}"
                            @else
                            src="{{ static_avatar($user_data) }}" @endif
                                onerror="this.onerror=null;this.src='{{ static_avatar($user_data) }}';">
                        </span>
